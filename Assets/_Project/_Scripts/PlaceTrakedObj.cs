...
public class PlaceTrakedObj : MonoBehaviour
{
    ...
    private void OnTrackedImagesChanged(ARTrackedImagesChangedEventArgs eventArgs)
    {
        foreach (var trackedImage in eventArgs.added)
        {
            ...
            foreach (var curPrefab in ArPrefabs)
            {
                if (string.Compare(curPrefab.name, imageName, StringComparison.OrdinalIgnoreCase) == 0 && !_instantiatedPrefabs.ContainsKey(imageName))
                {
                    ...
                    instantiatedObj = Instantiate(ArPrefabs[0], imagePos, imageRot);

                    instantiatedObj.transform.SetPositionAndRotation(imagePos, imageRot);
        ...
        {
            instantiatedObj.transform.position = trackedImage.transform.position;
            // AppManager.Instanse.PlaceCurrentModel(trackedImage.transform.position);
        }
    }
}
